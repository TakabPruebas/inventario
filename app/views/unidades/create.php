...
<head>
    <meta charset="UTF-8">
    <title><?= isset($unidad) ? 'Editar' : 'Agregar' ?> Unidad de Medida - TAKAB</title>
...
<div class="main-layout">
    ...
    <div class="content-area">
        ...
        <main class="dashboard-main">
            <div class="form-card">
                <div class="form-title"><?= isset($unidad) ? 'Editar' : 'Agregar' ?> Unidad de Medida</div>
                <?php if (!empty($error)) echo "<div class='form-error'>$error</div>"; ?>
                <form class="takab-form" method="post" action="">
                    <label>Nombre:</label>
                    <input type="text" name="nombre" value="<?= htmlspecialchars($unidad['nombre'] ?? '') ?>" required>
                    <label>Abreviación:</label>
                    <input type="text" name="abreviacion" value="<?= htmlspecialchars($unidad['abreviacion'] ?? '') ?>" required>
                    <button type="submit"><?= isset($unidad) ? 'Actualizar' : 'Guardar' ?></button>
                </form>
